...
@section('content')
    <div id="content">
        <div class="inner" style="min-height:700px;">
            ...
            <div class="row">
                <div class="col-lg-12">
                    {!! Form::model($record, ['method' => 'PATCH', 'route' => ['books.update', $record->id], 'class' => 'form-horizontal', 'files' => true]) !!}
                    <div class="row">
                        <div class="col-lg-12">
                                                    ...
                                                    <div class="well">
                                                        <h4 style="color: red">{{ $moalef->name_writers }}
                                                            ...
                                                            <input type="checkbox" <?php
                                                                if (getstatemoalef($record->id, $moalef->id)) {
                                                                    # code...
                                                ...
                                                <div class="col-lg-4">
                                                    <div class="well">
                                                        <h4 style="color: red">{{ $pakhshs->name_pakhsh }}
                                                            <input type="checkbox" <?php
                                                                if (getstatepakhsh($record->id, $pakhshs->id)) {
                                                                    # code...
                                                                    echo 'checked="checked';
                                                                }
                                                                ?> name="pakhshselect[]"
...
function getstatemoalef($book_id, $moalef_id)
{
    $checkbox = MoalefBooksModel::where(['id_books' => $book_id, 'id_moalef' => $moalef_id])->count();
    if ($checkbox != 0) {
        return true;
    } else {
        return false;
...
function getstatemotarjem($book_id, $motarjems_id)
{
    $checkbox = BookMotarjemModel::where(['id_book' => $book_id, 'id_motarjems' => $motarjems_id])->count();
    if ($checkbox != 0) {
        return true;
    } else {
        return false;
...
{
    $checkbox = PakhshBooksModel::where(['id_book' => $book_id, 'id_pakhsh' => $pakhsh_id])->count();
    if ($checkbox != 0) {
        return true;
    } else {
        return false;
